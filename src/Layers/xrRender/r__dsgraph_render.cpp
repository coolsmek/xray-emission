...
void CDSGraphManager::r_dsgraph_render_dynamic_range(u32 _priority, u32 passBegin, u32 passEnd, u32 packetBegin, u32 packetEnd)
{
    const bool dp = !!strstr(Core.Params, "-vk_draw_perf");
    u64 t_start = dp ? CPU::GetCLK() : 0;
    ...
    for (u32 iPass = passBegin; iPass < passEnd; ++iPass)
    {
        auto& queue = RGraph.mapDynamicPasses[_priority][iPass];
        if (queue.empty())
            continue;
