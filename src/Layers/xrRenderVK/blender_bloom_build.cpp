/*
Summary Strategy:
Leave blender_bloom_build.cpp completely alone! Keep the C.r_dx10Texture and C.r_dx10Sampler names active.
Because those names are just data-gathering functions, you can safely pull the strings and parameters they
register directly into your Vulkan descriptor layouts behind the scenes when initializing your pipeline passes.
1. Loop through all textures registered by C.r_dx10Texture
    The legacy pass structure holds the texture names and register slots
    Find your native Vulkan texture object by its string name
    Queue this vkTex->imageView up for your active VkDescriptorSet layout!
2. Loop through all samplers registered by C.r_dx10Sampler
    Parse state flags (linear filtering, anisotropy, wrapping modes)
    and fetch or create a matching VkSampler
    Bind vkSampler to the current descriptor set layout slot
*/
