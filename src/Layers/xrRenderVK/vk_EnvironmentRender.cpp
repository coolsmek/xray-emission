...
static u16 hbox_faces[20 * 3] =
{
	...
	0, 1, 9, 9, 8, 0, 8, 9, 5, 5, 4, 8,
	1, 3, 10, 10, 9, 1, 9, 10, 7, 7, 5, 9,
	3, 2, 11, 11, 10, 3, 10, 11, 6, 6, 7, 10,
	2, 0, 8, 8, 11, 2, 11, 8, 4, 4, 6, 11
};
...
{
	*this = *(vkEnvDescriptorRender*)&_in;
}

void vkEnvDescriptorMixerRender::Copy(IEnvDescriptorMixerRender& _in)
{
	vkEnvDescriptorMixerRender* other = (vkEnvDescriptorMixerRender*)&_in;
...
{
	vkEnvDescriptorRender* pA = (vkEnvDescriptorRender *)inA;
	vkEnvDescriptorRender* pB = (vkEnvDescriptorRender *)inB;
...
{
	vkEnvDescriptorMixerRender& mixRen = *(vkEnvDescriptorMixerRender*)&*g_pGamePersistent->Environment().CurrentEnv->m_pDescriptorMixer;
...
vkEnvironmentRender::vkEnvironmentRender()
{
	...
	tenv0 = DEV->_CreateTexture("$user$env_s0");
	tenv1 = DEV->_CreateTexture("$user$env_s1");
}

void vkEnvironmentRender::OnFrame(CEnvironment& env)
{
	PROF_EVENT("EnvironmentRender::OnFrame");
	vkEnvDescriptorMixerRender& mixRen = *(vkEnvDescriptorMixerRender*)&*env.CurrentEnv->m_pDescriptorMixer;
	...
	if (mixRen.sky_r_textures_env.size() >= 2)
	{
		// Populate the env cubemaps ($user$env_s0/1) that hmodel() samples in combine_1.ps
		// for hemispheric/ambient fill. Without this they stay as 1x1 null textures, so
		// env_d = 0 and hdiffuse collapses to L_ambient only → shadowed surfaces go black
		// (scene looks "lit by sun angle only").
		ID3DBaseTexture* ev0 = mixRen.sky_r_textures_env[0].second->surface_get();
		ID3DBaseTexture* ev1 = mixRen.sky_r_textures_env[1].second->surface_get();
		tenv0->surface_set(ev0);
		_RELEASE(ev0);
		tenv1->surface_set(ev1);
		_RELEASE(ev1);
	}

	// tonemapping in VS
	mixRen.sky_r_textures.push_back(mk_pair(u32(CTexture::rstVertex), tonemap));
	mixRen.sky_r_textures_env.push_back(mk_pair(u32(CTexture::rstVertex), tonemap));
	mixRen.clouds_r_textures.push_back(mk_pair(u32(CTexture::rstVertex), tonemap));
}
...
void vkEnvironmentRender::OnDeviceDestroy()
{
	...
	if (tenv0) tenv0->surface_set(NULL);
	if (tenv1) tenv1->surface_set(NULL);
