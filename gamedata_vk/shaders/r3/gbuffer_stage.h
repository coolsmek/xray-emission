...
// Unpacking stage for HUD geometry (uses pos_decompression_params_hud)
gbuffer_data gbuffer_load_data_hud( float2 tc : TEXCOORD, float2 pos2d, uint iSample )
{
	gbuffer_data gbd;
	...
	// 3d view space pos reconstruction math with HUD FOV decompression
	gbd.P = float3( P.z * ( pos2d * pos_decompression_params_hud.zw - pos_decompression_params_hud.xy ), P.z );
...
gbuffer_data gbuffer_load_data_hud( float2 tc : TEXCOORD, float2 pos2d )
{
   return gbuffer_load_data_hud( tc, pos2d, 0 );
}
